...
async function getVehicles(id) {
    if (!id) { const data = await readFileFunction(); return data.vehicles }
    id = parseInt(id)
...
async function patchVehicles(id) {
    ...
    if (vehicle === undefined) { throw new Error("pedido Não encontrado") } else {
...
async function updateVehicle({ name, description, plate, year, color, price, id }) {
    year = parseInt(year)
    price = parseInt(price)
    id = parseInt(id)
    let data = await readFileFunction()
