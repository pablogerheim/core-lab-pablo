...
async function createVehicle(req, res, next) {
    ...
    try {
        ...
        if (!name == null || description == null || plate == null || year == null || color == null || price == null) {
            return res.status(422).json({ msg: "O Nome, Descrição, Placa, Ano, Cor e Preço são obrigatórios!" });
        }
        const vehicle = await vehiclesService.createVehicle(req.body);
        res.status(200).json({ msg: "Criação realizada com sucesso!", vehicle });
...
async function updateVehicle(req, res, next) {
    try {
        ...
        if (!name == null || description == null || plate == null || year == null || color == null || price == null || id == null) {
            return res.status(422).json({ msg: "O Nome, Descrição, Placa, Ano, Cor, Preço e Id são obrigatórios!" });
        }
        const vehicle = await vehiclesService.updateVehicle(req.body);
        res.status(200).json({ msg: "Atualização realizada com sucesso!", vehicle });
