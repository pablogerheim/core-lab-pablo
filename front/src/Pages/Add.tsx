...
type CallbackId = (id:number) => void;

type IAdd = {
	id: number;
	setCards: CallbackCard | undefined;
	setId: CallbackId | undefined;
};
...
function Add({id, setCards, setId}: IAdd): JSX.Element {
	...
	async function handleSubmit(): Promise<void> {
		...
		if (id !== 0) {
			await api.put<IVehicle>('/', {name, color, description, year, plate, price, id});
		}

		if (setCards) {
			const dados = await api.get<IVehicle[]>('/'); setCards(dados.data);
		}
